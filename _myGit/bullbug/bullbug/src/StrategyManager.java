...
public class StrategyManager {
	...
	boolean isEXP;
	InformationManager myInstance;
	WorkerManager myWorker;
	int attack_cnt;
	...
	void setVariables(){
		...
		myInstance = new InformationManager();
		myWorker = new WorkerManager();
		attack_cnt = 0;
		...
		combatState = CombatState.defenseMode;
		 ...
		 if (myRace == Race.Terran) {
			...
			necessaryNumberOfCombatUnitType2 = 3;                       // 공격을 시작하기위해 필요한 최소한의 메딕 유닛 숫자 
	...
	public void setInitialBuildOrder() 
	{
	 ...
	 if (MyBotModule.Broodwar.self().getRace() == Race.Terran) 
	 {
      ...
      //BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Barracks);
		    ...
		    //BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryTechType1); //스팀팩
		    //BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryTechType2); //탱크모드 리서치
		    //BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryTechType3); //이레디에이트   
		    //BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryUpgradeType1); //마린 사거리업
		    //BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryUpgradeType2); //마린 공업
		 
		 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 1
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 2
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 3
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 4
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 5
         ...
         BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Barracks); // 9
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 15
		    ...
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 19	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 20
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Academy); // 43
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 24
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 8
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Refinery); //45
		    ...
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Engineering_Bay); //28
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 29
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 26	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 27	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 37

		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 30	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 31	
		    
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Bunker, seedPositionStrategyOfMyDefenseBuildingType); // 32
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 33
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 34	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 35	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryTechType1); //97 스팀팩
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 8

			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 26	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 27	
	        BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Comsat_Station, false); // 68 

		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Missile_Turret, BuildOrderItem.SeedPositionStrategy.SecondChokePoint, false); //36
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 37
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 38	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 39	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 26	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 27	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Barracks); // 40
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 29

		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 41	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 42	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryUpgradeType2); //마린 공업
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Barracks); // 52
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 29
		    ...
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 46	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 46	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 46	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 46	

			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 48
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 29

		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Missile_Turret, BuildOrderItem.SeedPositionStrategy.FirstChokePoint, false); // -
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Missile_Turret, BuildOrderItem.SeedPositionStrategy.FirstChokePoint, false); // -
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 49	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 50	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 51	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 46	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory); //102

	        BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 49	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 50	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 51	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 55
			
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 56
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 57
				BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 103 
				BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 104	
				BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 105	
				BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 106	
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 58
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 59 	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 60	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 61 
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 62 
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryTechType2); //탱크모드 리서치
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 59 	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 60	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 69
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 61 
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 62 
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 69
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
		    ...
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryUpgradeType1); //마린 사거리업
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 70	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 71
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 69
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 72	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 73 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 74
			
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Science_Facility); ///////////////////////////////////////////////
			
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Control_Tower,false); // 107 /////////////////////////////////////////
			
			
			
			
			
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 75	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 69
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 76	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 77	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 78 
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 79
			
			
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 80	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 81	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 82	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 83 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 79
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 69
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 84	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 85	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 86	
		    BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 87
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 79
...
//					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Comsat_Station, false);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 69
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 88
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 91	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 92	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 93	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 94 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 95

			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV); // 96
		    
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 97	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 98	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 99	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 100 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);

			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 107
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 103 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 104	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 105	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 106	
			...
			//BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Science_Facility);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); // 107
			//BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Control_Tower); // 107
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Medic); // 103 
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 104	
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Marine); // 105	
			...
			if(cnt==1)
			{
				...
				for(int i = 1 ; i < size ; i++)
				{
					if(MyBotModule.Broodwar.isExplored(startLocation.get(i).getTilePosition()))
					{
						...
						if (myInstance.hasBuildingAroundBaseLocation(startLocation.get(i), myPlayer, 200) == false)
						{
		      ...
		      for (Unit unit : myAllCombatUnitList) 
		      {
		         ...
		         boolean hasCommanded = false;
		            ...
		            for (Unit building : MyBotModule.Broodwar.self().getUnits())
		            {
		               ...
		               if (hasCommanded == false && building.getType().isBuilding() && building.isCompleted() == true && building.getHitPoints() < building.getType().maxHitPoints() && building.getType() != UnitType.Terran_Bunker && building.getType() != UnitType.Terran_Missile_Turret)
		               {
		            	   for(Unit enemyUnit : MyBotModule.Broodwar.getUnitsInRadius(building.getPoint(), 8 * Config.TILE_SIZE)) 
		       			{
		                  ...
		                  myAttackedBuildingPosition = setTargetPositionOne(myAttackedBuildingPosition,200);
		         ...
		         /*
			               ...
			               if (unit.getDistance(mySecondChokePoint.getCenter())>40) 
			               {
			                  commandUtil.move(unit, mySecondChokePoint.getCenter());
			               }
		...
		void commandMyCombatUnitToAttack(){
			...
			if (targetPoint != null) 
			{
				...
				*/
				...
				Position mapCenter = new Position (mapWidth/2, mapHeight/2);
				
				Position position01 = setTargetPositionTwo(mapCenter, enemySecondChokePoint.getPoint(), 1);
				position01 = setTargetPositionTwo(position01, enemySecondChokePoint.getPoint(), 1);
				position01 = setTargetPositionTwo(position01, enemySecondChokePoint.getPoint(), 1);
				position01 = setTargetPositionTwo(position01, enemySecondChokePoint.getPoint(), 15);
				...
				Position position04 = setTargetPositionOne(enemyFirstExpansionLocation.getPoint(), 45);
				Position position05 = setTargetPositionTwo(enemyFirstChokePoint.getPoint(), enemyMainBaseLocation.getPoint(), 45);
