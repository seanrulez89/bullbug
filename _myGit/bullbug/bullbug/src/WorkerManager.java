...
public class WorkerManager {
	...
	public void checkBuilding ()
	{
		...
		{
			if(unit.getType().isBuilding() && !unit.isBeingConstructed() && !unit.isCompleted())
			{
				System.out.println("일꾼없는 건물 발생");
				worker = chooseConstuctionWorkerClosestTo(unit.getType(), unit.getTilePosition(),true, -1);
				commandUtil.rightClick(worker, unit);
				setIdleWorker(worker);
//				setConstructionWorker(chooseConstuctionWorkerClosestTo(unit.getType(), unit.getTilePosition(),true, -1), unit.getType());
//				BuildManager.Instance().buildQueue.queueAsHighestPriority(unit.getType(), true, myWorker.chooseConstuctionWorkerClosestTo(unit.getType(), unit.getTilePosition(),true, -1).getID());
			}
		}
	}
	...
	public Unit chooseConstuctionWorkerClosestTo(UnitType buildingType, TilePosition buildingPosition, boolean setJobAsConstructionWorker, int avoidWorkerID)
	{
		...
		for (Unit unit : workerData.getWorkers())
		{
			...
			// 추가 수정 : Scout Worker 중에서는 차출하지 않도록 한다
			if (unit.isCompleted() 
